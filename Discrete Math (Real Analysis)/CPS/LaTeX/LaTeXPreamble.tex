...
\usetikzlibrary{matrix, shapes, positioning, calc, decorations.pathreplacing, shapes.geometric, arrows, fpu, fixedpointarithmetic, patterns}
...
\newcommand{\CalcABQuizTitle}[1]{
	...
	\begin{center}\fbox{\fbox{\parbox{\textwidth}{\ifstrequal{#1}{}{No calculator is allowed for these problems.}{A graphing calculator is required for these problems. Your final answers should be accurate to three places after the decimal point.} Show all of your work. Clearly label any functions, graphs, tables, or other objects that you use. Answers without supporting work will usually not receive credit. There are \total{PointsPossible} points possible. }}}\end{center}
	\thispagestyle{empty}
}

\newcommand{\CalcDiscGroupQuizTitle}[1]{
	\getmainfile	
	{\fontsize{15}{18}\selectfont\textbf{AP Calculus AB - Discussion}} \hfill \textbf{Names:} \mun{2} \\ [12pt]
	{\fontsize{15}{18}\selectfont\textbf{Group Quiz: Mahavier \StrLeft{\themainfile}{3}}} \hfill \textbf{Period:} \mun{2} \\ \vspace{-20pt}
	\begin{center}\fbox{\fbox{\parbox{\textwidth}{There are \total{PointsPossible} points possible. Check the boxes below to indicate that you understand the rules and guidelines of this activity.
	\begin{itemize}
		\item[$\square$] It is not acceptable to split up the problems. Every member of the group should be working together on the same problem at all times.
		\item[$\square$] \ifstrequal{#1}{}{No calculator is allowed for these problems.}{A graphing calculator is required for these problems. \item[$\square$]  \points{2}{1}Your final answers should be accurate to three places after the decimal point.}
		\item[$\square$] You may access our textbook and your notes. However, you may not access any other resources (i.e., no Googling, asking non-group-members for help, etc.). This means members of one group may not communicate with members of a different group.
		\item[$\square$] Show all of your work. Answers without supporting work will usually not receive credit. 
		\item[$\square$] I will grade this for the clarity of your explanations in addition to the correctness of your work. Clearly label any functions, graphs, tables, or other objects that you use.
		\item[$\square$] Do not submit a rough draft. Make sure that your work is readable and organized.
	\end{itemize}		
	}}}\end{center}
	\thispagestyle{empty}
}

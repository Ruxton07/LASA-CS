...
public class WordSearch
{
    ...
    //checking diagonals
    for (int i = 0; i < table.length; i ++) {
      for (int j = 0; j < table[i].length; j++) { //checking all values
        if (table[i][j] == s.charAt(0)) {
        int a;
        for ( a = 1; a < s.length(); a++) { 
          //will check diagonally to top right
          if ( (i - a) < 0 || j + a > table[0].length - 1 ) 
            break;
          
          if ((table[i-a][j+a] != s.charAt(a)))
            break;
        }
          if (a == s.length()) 
          return true;
        ...
        for (a = 1; a < s.length(); a++) {
          if ((i + a) > table.length - 1 || j + a > table[0].length - 1) {
            break;
